<?php
  ...
  if(isset($_GET["id"])){
    ...
    $sql="SELECT* FROM Vanchuyen_Huyen WHERE Mahuyen='$Mahuyen'";
    $result=$conn->query($sql);
    //Kiểm tra tồn tại
    if($result->num_rows>0){
      $row = $result->fetch_assoc();
      //Nhan du lieu
      $txtTenhuyen=$row["Tenhuyen"];
      $txtPhivanchuyen=$row["PhiVanchuyen"];
    }
    $conn->close();
  }
?>
  ...
  <div style="margin-left: 20px; margin-right: 20px;">
    ...
    <div class="btn-group" style="margin-top: 10px;">
      ...
      <button type="button" class="btn btn-outline-primary"
        onclick="location.reload();">Cập nhật</button></div>
    <!--Bảng-->
      ...
      <div class="col-sm-9">
        <div class="card">
          <div class="card-header" style="background-color: green; border: #F5CFCF; color: white;">
            <h4 style="font-weight: bold;">Cập nhật Huyện, Thành phố</h4>
          </div>
          <div class="card-body row">
            <div class="col-sm-4"></div>
            <div class="col-sm-4">
              <!------------------------------------------->
              <form action="xuly_vanchuyenhuyen.php?tacvu=sua&id=<?php echo $Mahuyen; ?>" method="post">
                <!--Nhập tên huyện-->
                <div class="form-group">
                  <label style="font-weight: bold;">Tên huyện, thành phố:</label>
                  <input type="text" class="form-control" placeholder="Nhập tên huyện, thành phố" name="txtTenhuyen" value="<?php echo $txtTenhuyen; ?>">
                </div>
                <!--Nhập phí vận chuyện-->
                <div class="form-group">
                  <label style="font-weight: bold;">Phí vận chuyển:</label>
                  <input type="text" class="form-control" placeholder="Nhập phí vận chuyển" name="txtPhivanchuyen" value="<?php echo $txtPhivanchuyen; ?>">
                </div>
                <!--Button-->
                <div align="center">
